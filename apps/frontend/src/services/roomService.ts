...
// ─── Интерфейс транспорта (независим от реализации) ────────────────────────
...
export interface RoomTransport {
  ...
  onRoomState(cb: (state: { tokens: Record<string, Token> }) => void): void
...
// ─── Реализация на Socket.io (для веба) ────────────────────────────────────
...
export class SocketIOTransport implements RoomTransport {
  ...
  joinRoom(roomId: string, userId: string | null, username: string | null): void {
    console.log('=== joinRoom called ===', { roomId, userId, username })
