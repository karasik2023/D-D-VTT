  ...
  return (
    <div style={styles.page}>
      <div style={styles.card}>
        ...
        <h2 style={styles.subtitle}>Регистрация Мастера</h2>
        {error && <p style={styles.error}>{error}</p>}
        <input style={styles.input} placeholder="Имя мастера" value={username}
        ...
        <button style={styles.button} onClick={handleSubmit}>Создать аккаунт</button>
        <p style={styles.link}>Уже есть аккаунт? <Link to="/login" style={{ color: '#a78bfa' }}>Войти</Link></p>
      </div>
    </div>
  )
}
...
page: { background: '#1a1a2e', minHeight: '100vh', width: '100vw', display: 'flex', alignItems: 'center', justifyContent: 'center' },  card: { background: '#16213e', padding: 40, borderRadius: 12, width: 360, display: 'flex', flexDirection: 'column', gap: 12 },
