...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    return () => { socket.off('connect'); socket.off('token-move'); socket.off('room-state')}
  }, [roomId])
  ...
  return (
    <div style={{ background: '#1a1a2e', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 20px', borderBottom: '1px solid #1e293b' }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button onClick={() => navigate('/')} style={{ background: 'transparent', border: '1px solid #334155', borderRadius: 6, padding: '6px 12px', color: '#94a3b8', cursor: 'pointer', fontSize: 13 }}>
            ← Лобби
          </button>
        </div>
      </div>
      <Stage ref={stageRef} width={window.innerWidth} height={window.innerHeight - 50} draggable onWheel={handleWheel}>
